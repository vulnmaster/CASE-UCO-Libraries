// Auto-generated CASE/UCO ontology class — do not edit manually.
package org.caseontology.case.investigation;

import java.util.ArrayList;
import java.util.List;

/** A subject action lifecycle is an action pattern consisting of an ordered set of multiple actions or subordinate action-lifecycles performed by an entity acting in a role whose conduct may be within th */
public class SubjectActionLifecycle extends ActionLifecycle {
    public static final String CLASS_IRI = "https://ontology.caseontology.org/case/investigation/SubjectActionLifecycle";
    public static final String NAMESPACE_PREFIX = "case-investigation";


    public SubjectActionLifecycle() {
    }

}